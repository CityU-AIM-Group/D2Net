...
class DMFUnit(nn.Module): 
    ...
    def __init__(self, num_in, num_out, g=1, stride=1,norm=None,dilation=None):
        ...
        self.cond_weight1 = nn.Parameter(torch.Tensor(num_in,1))
        num_mid = num_in if num_in <= num_out else num_out # 2
        
        self.conv1x1x1_in1 = Conv3d_Block(num_in, num_in // 4, kernel_size=1, stride=1, norm=norm) # BN+Relu+Conv
        self.conv1x1x1_in2 = Conv3d_Block(num_in // 4,num_mid,kernel_size=1, stride=1, norm=norm)  # BN+Relu+Conv
...
class DMFNet(MFNet): # softmax
    ...
    def __init__(self, in_channels=4, channels1=32, channels2=128, groups=16, norm='bn', num_classes=4):
        super(DMFNet, self).__init__()
        self.encoder_block2 = nn.Sequential(
            DMFUnit(channels1, channels2, g=groups, stride=2, norm=norm,dilation=[1,2,3]),# H//4 down
            DMFUnit(channels2, channels2, g=groups, stride=1, norm=norm,dilation=[1,2,3]), # Dilated Conv 3
            DMFUnit(channels2, channels2, g=groups, stride=1, norm=norm,dilation=[1,2,3])
        )

        self.encoder_block3 = nn.Sequential(
            DMFUnit(channels2, channels2*2, g=groups, stride=2, norm=norm,dilation=[1,2,3]), # H//8
            DMFUnit(channels2 * 2, channels2 * 2, g=groups, stride=1, norm=norm,dilation=[1,2,3]),# Dilated Conv 3
            DMFUnit(channels2 * 2, channels2 * 2, g=groups, stride=1, norm=norm,dilation=[1,2,3])
        )
